...
class TheaterControllerTest {
    ...
    @Test
    void updateTheater_shouldReturnUpdatedTheater() throws Exception {
        TheaterDTO requestDTO = new TheaterDTO();
        requestDTO.setId(1L);
        requestDTO.setName("Updated Theater Name");
        requestDTO.setAddress("123 Kenmount Rd");
        requestDTO.setCity("St. John's");
        requestDTO.setState("NL");
        requestDTO.setPostalCode("A1B 1A1");
        requestDTO.setPhoneNumber("709-722-1234");

        Theater updatedTheater = new Theater();
        updatedTheater.setId(1L);
        updatedTheater.setName("Updated Theater Name");
        updatedTheater.setAddress("123 Kenmount Rd");
        updatedTheater.setCity("St. John's");
        updatedTheater.setState("NL");
        updatedTheater.setPostalCode("A1B 1A1");
        updatedTheater.setPhoneNumber("709-722-1234");

        given(theaterService.getTheaterById(eq(1L))).willReturn(Optional.of(theater1));
        given(theaterService.saveTheater(any(Theater.class))).willReturn(updatedTheater);
        ...
        mockMvc.perform(put("/api/theaters/{id}", 1L)
                ...
                .andExpect(status().isOk())
                .andExpect(jsonPath("$.id", is(1)))
                .andExpect(jsonPath("$.name", is("Updated Theater Name")));
    ...
    void deleteTheater_shouldReturnNoContent() throws Exception {
        given(theaterService.getTheaterById(eq(1L))).willReturn(Optional.of(theater1));
        doNothing().when(theaterService).deleteTheater(eq(1L));

        mockMvc.perform(delete("/api/theaters/{id}", 1L))
                .andExpect(status().isNoContent());

        verify(theaterService, times(1)).getTheaterById(eq(1L));
        verify(theaterService, times(1)).deleteTheater(eq(1L));
    }

    @Test
    void deleteTheater_whenNotExists_shouldReturnNotFound() throws Exception {
        given(theaterService.getTheaterById(eq(99L))).willReturn(Optional.empty());

        mockMvc.perform(delete("/api/theaters/{id}", 99L))
                .andExpect(status().isNotFound());

        verify(theaterService, times(1)).getTheaterById(eq(99L));
